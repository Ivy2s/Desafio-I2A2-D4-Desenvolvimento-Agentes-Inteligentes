...
export function QueryComposer({ value, busy, error, onChange, onSubmit }: QueryComposerProps) {
  return <form className="query-composer" onSubmit={(event) => { event.preventDefault(); if (value.trim()) onSubmit(value.trim()) }}>
    ...
    <div className="query-input-wrap">
      <textarea id="query-input" value={value} maxLength={500} onChange={(event) => onChange(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter' && !event.shiftKey) { event.preventDefault(); if (value.trim()) onSubmit(value.trim()) } }} placeholder="O que você quer descobrir nos seus dados?" rows={3} disabled={busy} aria-describedby={error ? 'query-error' : 'query-help'} />
    ...
    <div className="composer-foot"><span id={error ? 'query-error' : 'query-help'} className={error ? 'query-error' : ''} role={error ? 'alert' : undefined}>{error || 'Enter para enviar · Shift + Enter para nova linha'}</span><span>{value.length}/500</span></div>
  </form>
}
