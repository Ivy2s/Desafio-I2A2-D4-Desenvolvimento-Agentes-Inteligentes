...
| REQ-12 | Resposta correta | operações Pandas e contrato tipado | resposta real = 7 total, 4 em compras; quatro perguntas completas não concluídas | FAIL |
| REQ-13 | Resposta em texto | campo `answer` na UI | E2E query real | PASS |
| REQ-14 | Resposta em tabela | `TableData` e `DataTable` | testes frontend/backend; agente real pendente | FAIL |
| REQ-15 | Resposta em gráfico | gráfico SVG derivado de tabela | testes de derivação; agente real pendente | FAIL |
| REQ-16 | Framework do curso | LangChain + Gemini | requirements/imports/relatório | PASS |

Os estados FAIL são bloqueadores de certificação porque o provedor Gemini
atingiu timeout ao tentar a segunda pergunta real. Não são ocultados por mocks.
